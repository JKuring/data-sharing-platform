...
/**
  ...
  */
class SssHbaseExecutor(configServiceUrl: String, timeid: String) extends SssExecutor(configServiceUrl, timeid) {
  ...
  def executor(node: SssNode) = {
    ...
    val result = sqlContext.sql(Context.getSql(configServiceUrl, node.getTplCiCode, stat_month, stat_date, stat_hour, stat_minute, timeid, node))
