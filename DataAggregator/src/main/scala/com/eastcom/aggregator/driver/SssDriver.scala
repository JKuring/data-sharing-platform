...
class SssDriver(val job: SssJob, val mqConf: MQConf, val headProperties: Array[String]) extends Thread with Actor {
  ...
  val workerRouter = context.actorOf(Props(new SssManager(job.configServiceUrl, job.timeid, mqConf, headProperties)).withRouter(RoundRobinRouter(job.sessions)), "sssWorkerRouter")
