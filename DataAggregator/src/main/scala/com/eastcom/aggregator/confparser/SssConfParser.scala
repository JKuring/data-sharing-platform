...
object SssConfParser {
  // conf
  def parser(confFile: String, initCmdPath: String, configServiceUrl: String, sessions: Int, timeid: String): SssJob = {
    val job = new SssJob(initCmdPath, configServiceUrl, sessions, timeid)
...
//    })
    ...
    // http
    HttpRequestUtils.httpGet(configServiceUrl + confFile, "".getClass).split("\\n").foreach(config => {
      if (config != null && config.trim != "" && !config.startsWith("#")) {
        job + new SssNode(config)
      }
    })
