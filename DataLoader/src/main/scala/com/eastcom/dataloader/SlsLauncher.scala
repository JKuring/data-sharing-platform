...
object SlsLauncher {
  ...
  def main(args: Array[String]): Unit = {
    ...
    val Array(confFile, initCmdPath, configServiceUrl, zookeeper_hosts, zookeeper_port, timeid) =
    if (args == null || args.isEmpty) {
      ...
      throw new SlsException("parameter list ( confFile , udfPath , configServiceUrl , zookeeper_hosts , zookeeper_port)")
    } else {
      args
    }
    ...
    //根据配置文件来创建多个任务的job
    val sohJob = SlsConfParser.parser(confFile, initCmdPath, configServiceUrl)
