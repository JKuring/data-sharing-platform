...
/**
  ...
  */
class SlsManager(configServiceUrl: String) extends Actor {
  val logging = Logger.getLogger(getClass)
  val hiveExecutor = new SlsHiveExecutor(configServiceUrl)
  val hbaseExecutor = new SlsHbaseExecutor(configServiceUrl)
